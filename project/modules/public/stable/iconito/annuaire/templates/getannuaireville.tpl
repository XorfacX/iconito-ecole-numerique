...
{if $ville.blog}<div class="right"><a title="{$ville.blog}" href="{$ville.blog}">{i18n key="annuaire.blogVille"}</a></div>{/if}
...
<div class="ecole_web">{if $ecole.blog}<a title="{$ecole.blog}" href="{$ecole.blog}">{i18n key="annuaire.blog"}</a>{/if}

<a title="{i18n key="annuaire.fiche"}" href="{copixurl dest="fichesecoles||fiche" id=$ecole.id}">{i18n key="annuaire.fiche"}</a>
