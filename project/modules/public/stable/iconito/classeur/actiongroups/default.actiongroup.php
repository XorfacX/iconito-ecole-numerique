...
// upload_fs
                ...
                if (!empty($uploadedFiles)) {
                    foreach ($uploadedFiles as $folder) {
                        foreach ($folder['files'] as $file) {
                            $fichier = ClasseurService::uploadFile($ppo->dossierTmp.'/'.$file, $file, $classeur, $ppo->dossier);
                            $this->sendNotifications($fichier->fichier, $classeur);
                        }
                    }
                } else {
                            ...
                            foreach ($datas as $dossierParent => $data) {
                                ...
                                if (!empty($data['files'])) {
                                    foreach ($data['files'] as $file) {
                                        ...
                                        $this->sendNotifications($fichier->titre, $classeur);
                                    }
                                }
                            }
                        }
    ...
    private function sendNotifications($fileTitle, $classeur) {
        $title = 'Nouveau fichier dans ' . $classeur->titre;
        $classeurUrl = CopixUrl::get ('classeur||voirContenu', array('classeurId' => $classeur->id));
        $body = 'Un nouveau fichier (' . $fileTitle . ') a été ajouté dans le classeur ' . $classeur->titre . ' : (<a href="' . $classeurUrl . '">lien vers le classeur</a>) ';
        $users = ClasseurService::getAbonnementUsers('CLASSEUR_ADD_FILE', $classeur->id);
        ClasseurService::sendNotifications($title, $body, $users);
    }
    ...
    /**
     * ABONNEMENT - Edition de l'abonnement au classeur
     */
    public function processEditerAbonnement()
    {
        $ppo = new CopixPPO();
        $ppo->conf = new CopixPPO();

        $ppo->classeurId = _request('classeurId');
        ...
        // Soumission du formulaire
        if (CopixRequest::isMethod ('post')) {
            $active = (_request('subscribe', 0)) ? 1 : 0;
            ClasseurService::setAbonnementStatus('CLASSEUR_ADD_FILE', $ppo->classeurId, _currentUser()->getId(), $active);

            return _arRedirect (CopixUrl::get ('classeur||editerAbonnement', array('classeurId' => $ppo->classeurId, 'success' => true)));
        } else {
            $ppo->abonnementStatus = ClasseurService::getAbonnementStatus('CLASSEUR_ADD_FILE', $ppo->classeurId, _currentUser()->getId());
        }

        $modParentInfo = Kernel::getModParentInfo('MOD_CLASSEUR', $ppo->classeurId);
        $ppo->TITLE_PAGE = $modParentInfo['nom'];

        return _arPPO ($ppo, array ('template' => 'editer_abonnement.tpl'));
    }
