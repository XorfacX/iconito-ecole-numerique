...
{elseif $ppo->nodeType == 'BU_ECOLE'}
    {if ($ppo->restricted_rights && $ppo->user->testCredential ("group:[Admin]")) || (!$ppo->restricted_rights && $ppo->user->testCredential ("module:school|`$ppo->nodeId`|classroom|create@gestionautonome") )}
...
{elseif $ppo->nodeType == 'BU_CLASSE'}
    ...
    {/if}
    {if ($ppo->restricted_rights && $ppo->user->testCredential ("group:[Admin]")) || (!$ppo->restricted_rights && $ppo->user->testCredential ("module:classroom|`$ppo->nodeId`|classroom|delete@gestionautonome") )}
